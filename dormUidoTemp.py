...
def dorm(logvec, correct=False):
  ...
  if correct:
    ...
    #uses // below to return an integer, so that python doesn't barf at having to divide by a number that is too big and return a too big floating point number
...
#In case you want to calculate uido without starting with words, but rather with a vector of info content values:

def uidoraw(logvec, lenCorrect = False):
